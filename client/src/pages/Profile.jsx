    ...
    <Container maxWidth="lg">
      <ProfileHeader elevation={3}>
        <Grid container spacing={3} alignItems="center">
          ...
          <Grid item>
            <Button variant="contained" color="secondary" sx={{ borderRadius: 28 }}>
              Edit Profile
            </Button>
          </Grid>
        </Grid>
      ...
      <Paper elevation={2} sx={{ mb: 4 }}>
        ...
        >
          <StyledTab icon={<AccountCircleIcon />} iconPosition="start" label="Overview" />
          <StyledTab icon={<PlaylistPlayIcon />} iconPosition="start" label="Playlists" />
          <StyledTab icon={<VideoLibraryIcon />} iconPosition="start" label="Saved Videos" />
        </Tabs>
              ...
              <Grid item xs={12} md={8}>
                <Paper elevation={1} sx={{ p: 3 }}>
                  ...
                  {playlists.slice(0, 3).map((playlist) => (
                    <Box key={playlist.id} mb={2} display="flex" alignItems="center">
                ...
                playlists.map((playlist) => (
                  <PlaylistCard key={playlist.id}>
                    <CardMedia
                      component="img"
                      sx={{ width: 140 }}
                      image={playlist.snippet.thumbnails?.medium?.url || 'https://via.placeholder.com/140x80?text=No+Thumbnail'}
                    ...
                    <CardContent sx={{ flex: '1 0 auto' }}>
                      ...
                      <Box mt={1}>
                        <Chip 
                          size="small" 
                          label={`Created: ${new Date(playlist.snippet.publishedAt).toLocaleDateString()}`} 
                          variant="outlined" 
                          sx={{ mr: 1 }} 
                        />
                      </Box>
