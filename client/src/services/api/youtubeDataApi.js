...
const createYTDataInstance = (accessToken = null) => {
  // If no access token is provided, fall back to API key
  ...
  instance.interceptors.response.use(
    (response) => response,
    async (error) => {
      // If error is authentication related and we have a token
      if (error.response?.status === 401 && accessToken) {
        // Clear invalid token
        // localStorage.removeItem("access_token");

        // Either redirect to login or try with API key as fallback
        if (API_KEY) {
          // Retry the request with API key instead
          const originalRequest = error.config;
          originalRequest.headers = { "X-Goog-Api-Key": API_KEY };
